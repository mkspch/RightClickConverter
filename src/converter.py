...
# The HTML template for image comparison
HTML_COMPARE_TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
def convert_exr_to_srgb_tif_sequence(first_file_path):
    """
    Converts an EXR image sequence (ACEScg) to an sRGB TIF image sequence,
    applying OCIO color management, keeping 32-bit float and alpha.
    ...
    """
    if not OCIO or not OIIO:
        print("Error: PyOpenColorIO or OpenImageIO not available. Cannot perform EXR to TIF conversion.")
        return False
    ...
    output_base_dir = os.path.dirname(first_file_path)
    base_name = os.path.basename(sequence_pattern).split('%')[0].rstrip('._-')
    output_sequence_dir = os.path.join(output_base_dir, f"{base_name}_sRGB_TIF_32bit")
    os.makedirs(output_sequence_dir, exist_ok=True)
    ...
    print(f"Starting color conversion of EXR sequence to 32-bit sRGB TIF sequence in {output_sequence_dir}...")
    ...
    try:
        for i, exr_path in enumerate(exr_files):
            ...
            # Apply OCIO color conversion (ACEScg to sRGB)
            # OIIO.ImageBufAlgo.colorconvert handles the color conversion.
            # It typically operates on the color channels and leaves alpha intact.
            success_ocio = OIIO.ImageBufAlgo.colorconvert(img_buf, img_buf, "ACEScg", "Output - sRGB", colorconfig=ocio_config_path)
            if not success_ocio:
                print(f"OCIO Color Convert failed for frame {start_frame + i}. Check OCIO config and colorspace names.")
                return False
            ...
            # Construct output filename
            frame_num_str = str(start_frame + i).zfill(len(str(len(exr_files) + start_frame -1)))
            output_tif_path = os.path.join(output_sequence_dir, f"{base_name}_{frame_num_str}.tif")
            
            # Save as TIF, explicitly requesting FLOAT (32-bit)
            success_write = img_buf.write(output_tif_path, OIIO.FLOAT)
            if not success_write:
                print(f"Error: Failed to write TIF file {output_tif_path}")
                return False

        print(f"Successfully converted EXR sequence to 32-bit sRGB TIF sequence in {output_sequence_dir}")
        return True

    except Exception as e:
        print(f"An error occurred during the EXR to TIF conversion process: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def generate_image_comparison(img_a_path, img_b_path, output_path=None):
    """
    Generates an HTML comparison tool for two images.
    """
    def process_image_b64(img_path, target_size=None, quality=90):
        try:
    ...
    if not output_path:
        output_dir = os.path.dirname(img_a_path)
        name_a = os.path.splitext(os.path.basename(img_a_path))[0]
        name_b = os.path.splitext(os.path.basename(img_b_path))[0]
        output_path = os.path.join(output_dir, f"compare_{name_a}_vs_{name_b}.html")

    print(f"Generating comparison: {os.path.basename(img_a_path)} vs {os.path.basename(img_b_path)}")
    ...
    img_a_b64 = process_image_b64(img_a_path, target_size=target_size)
    img_b_b64 = process_image_b64(img_b_path, target_size=target_size)

    if not img_a_b64 or not img_b_b64:
        return False

    html_content = HTML_COMPARE_TEMPLATE.format(
    ...
    try:
        ...
        print(f"Successfully created comparison: {output_path}")
        return True
    except Exception as e:
